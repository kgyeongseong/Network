...
int main(int argc, char **argv)
{
    ...
    // 첫 번째 인자 : Packet Capture Descriptor(패킷 캡처 지정자)
    // 두 번째 인자 : bfp_program 구조체의 포인터이며 pcap_compile 에 의해서 채워진다.
    // 세 번째 인자 : optimize (?)
    // 네 번째 인자 : 로컬 네트워크의 netmask
    ...
    // pcap_compile 을 통해서 지정된 필터를 적용시키기 위해서 사용되며,
    // 앞으로 들어오는 패킷에 대해서는 이 필터룰에 의해서 필터링 된다.
    ...
    // 첫 번째 인자 : PCD (Packet Capture Descriptor, 패킷 캡쳐 지정자)
    // 두 번째 인자 : 패킷 캡쳐를 몇 번에 걸쳐서 할 것인지를 결정
    // 세 번째 인자 : 패킷이 들어왔을 때 실행하는 함수의 포인터
